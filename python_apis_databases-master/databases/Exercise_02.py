...
5) See how many actors participated in each comedy movie (using group)
6) Select all the comedic films and sort them by rental rate (using order by)
"""))
...
if question==5:
    ...
    film_category=sqlalchemy.Table('film_category', metadata, autoload=True, autoload_with=engine)
    join_statement=actor.join(film_actor, film_actor.columns.actor_id==actor.columns.actor_id).join(film, film_actor.columns.film_id==film.columns.film_id).join(film_category, film_category.columns.film_id== film.columns.film_id)
    query3=sqlalchemy.select([film.columns.title, sqlalchemy.func.count(actor.columns.actor_id)]).where(film_category.columns.category_id==5).select_from(join_statement).group_by(film.columns.title)
